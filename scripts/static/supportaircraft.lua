...
function SpawnSupport (SupportSpawn) -- spawnobject, spawnzone, callsignName, callsignNumber
  if GROUP:FindByName(SupportSpawn.spawnobject) then
    ...
    SupportSpawnObject:InitLimit( 1, 0 )
            ...
            function ()
              if SpawnGroup then
                if SpawnGroup:IsNotInZone( ZONE:FindByName(SupportSpawn.spawnzone) ) then
                  ...
                  BASE:T("[SUPPORTSPAWN] Spawned aircraft: " .. SpawnGroup:GetName() .. " is not in zone.")
                end
              end
            end,
            {}, 0, 60 
      ...
      :InitRepeatOnLanding()
      :Spawn()
    BASE:T("[SUPPORTSPAWN] Spawned " .. SupportSpawn.spawnobject)
  else
    BASE:E("Function SpawnSupport: spawn template not found in mission: " .. tostring(SupportSpawn.spawnobject))
  end
end
