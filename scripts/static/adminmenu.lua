...
function ADMIN:LoadMission(playerName, missionFile)
  ...
  if playerName then
    BASE:E("ADMIN Restart or load called by player name: " .. playerName)
  else
    BASE:E("ADMIN Restart or load called by non-player!")
  end
  if missionFile then
...
function ADMIN:BuildAdminMenu(unit,playername)
  ...
  adminMenu = MENU_GROUP:New(adminGroup, "Admin", MISCroot)
  ...
  if ADMIN.missionList then
    BASE:T("ADMIN Build missionList.")
